...
const useResponsiveDimension = (responsive, config, key) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const breakpointKey = key[0].toUpperCase() + key.slice(1);

      if (width <= 480 && config[`mobile${breakpointKey}`]) {
        nextValue = config[`mobile${breakpointKey}`];
      } else if (width <= 768 && config[`tablet${breakpointKey}`]) {
        nextValue = config[`tablet${breakpointKey}`];
      } else if (width <= 1024 && config[`desktop${breakpointKey}`]) {
        nextValue = config[`desktop${breakpointKey}`];
      }
